...
func CalCpfp(txID string, childTxSizeVb float64, desiredFeeRate float64) (string, float64, error) {
	...
	if err != nil {
		return "", 0, fmt.Errorf("error getting transaction details: %s", err)
	}

	log.Info().Fields(map[string]interface{}{"action": "get tx details", "data": details, "txId": txID}).Send()

	if details.Status.Confirmed {
		return "", 0, fmt.Errorf("the transaction is already confirmed. ")
	}

	data, err := getCpfpHistory(txID)
	if err != nil {
		fmt.Println("Error calling API:", err)
		return "", 0, fmt.Errorf("get cpfp history error: %s", err.Error())
	} else if data.Error != "" {
		return "", 0, fmt.Errorf("invalid cpfp response: %s", data.Error)
	}

	log.Info().Fields(map[string]interface{}{"action": "get cpfp history", "data": data}).Send()

	if len(data.Ancestors) == 0 && len(data.Descendants) == 0 {
		return "", 0, fmt.Errorf("The transaction has already been confirmed or has no unconfirmed ancestors. ")
	}

	var decentTxId string
	if len(data.Descendants) > 0 {
		log.Info().Fields(map[string]interface{}{"action": "tx has decent", "tx": data.Descendants}).Send()
	...
	log.Info().Msgf("Recommended fee rate for new transaction D (in sats/vB): %.2f\n", recommendedFeeRateForD)

	return decentTxId, recommendedFeeRateForD, nil
}
