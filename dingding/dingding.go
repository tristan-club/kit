...
var defaultBot *Robot

type Robot struct {
	token, secret, appkey, secretKey string
}

func init() {
	token, secret := config.GetDingConfig()
	if len(token) != 0 || len(secret) != 0 {
		defaultBot = NewRobot(token, secret, "", "")
	}
}

func NewRobot(token, secret, appKey, secretKey string) *Robot {
	return &Robot{
		token:     token,
		secret:    secret,
		appkey:    appKey,
...
func Default() *Robot {
	...
	log.Error().Msgf("dingding bot config error")
	return &Robot{}
}
